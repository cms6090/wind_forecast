...
## 다음 할 일 (우선순위순)
1. **[다음 최우선] 피처셋 격차 검토(50개 vs 외부 179개)**: 외부 파이프라인은 SCADA로 학습한 물리 파워커브 예측치(`pc_pred_*`)를 피처로 써서 feature importance 23.6%를 차지할 만큼 효과를 봤는데(`phase3_model_selection.md` §3-7), 우리는 이런 물리+ML 하이브리드 피처가 없음. **leakage-guard 먼저**: SCADA 원시값은 test 기간에 없으므로, train 기간 SCADA로만 파워커브(풍속→발전량 곡선)를 fit하고, 예측기준시점 이후엔 예보 풍속을 그 곡선에 대입하는 구조(SCADA 자체를 피처로 쓰지 않음)인지 설계 단계에서 확인 후 진행. `wind-domain-features`/`feature-selection` 스킬 참고
