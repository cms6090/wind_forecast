...
9. **분위수 회귀 + actual 가중 학습(9절)**: 외부 참고자료(동일 대회를 다룬 별도 파이프라인 기록)에서 얻은 아이디어 — `src/metric.py`를 다시 보면 채점 대상 여부(`valid = actual >= capacity*0.10`)가 실측값 기준이라, 진짜 추정해야 할 값은 `E[y|x]`가 아니라 `E[y|x, y≥10%용량]`(항상 더 큰 값)이라는 구조적 특징이 있다. 이를 근거로 (a) CatBoost `Quantile:alpha=τ` 손실로 τ>0.5를 스윕해 예측을 위쪽으로 겨냥, (b) FICR이 `actual` 가중합이라는 점에 맞춰 학습 표본 가중치를 `actual`(kWh)로 부여 — 두 실험을 하이퍼파라미터 탐색과 분리해서(`DEFAULT_PARAMS` 고정) 순수 효과만 확인. seed 3개 재검증까지 포함.
...
**분위수 회귀 + actual 가중 학습 결과** (9절):

| 구분 | 3-fold 평균 Score |
|---|---|
| 기본값(가중 없음, MAE) | |
| actual 가중(MAE, τ=0.5) | |
| actual 가중 + 최적 τ(seed 평균) | |
...
- 2차 좁은 범위 탐색이 필요한지
- 분위수 회귀·actual 가중 학습이 유의미한 개선을 보이는지 — 보인다면 하이퍼파라미터 탐색 결과와 합쳐 재검증
- `train.ipynb`에 하드코딩할 최종 파라미터(하이퍼파라미터 + 손실함수/가중치 설정) 확정
